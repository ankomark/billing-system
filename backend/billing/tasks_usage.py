...
from billing.models import Customer, PPPoEUsageState, PPPoEUsageRecord
from billing.router_service import get_pppoe_live_usage_any_router


def collect_pppoe_usage(interval_seconds=300):
    """
    Run every 5 minutes (300s).
    Creates PPPoEUsageRecord rows (deltas) so graphs are stable.
    """
    now = timezone.now()
    start = now - timezone.timedelta(seconds=interval_seconds)

    customers = Customer.objects.filter(status="active", connection_type="pppoe")

    for c in customers:
        router, live = get_pppoe_live_usage_any_router(c)
        ...
        with transaction.atomic():
            state, _ = PPPoEUsageState.objects.select_for_update().get_or_create(customer=c)

            # If counters reset (reconnect), delta would be negative → clamp to 0
