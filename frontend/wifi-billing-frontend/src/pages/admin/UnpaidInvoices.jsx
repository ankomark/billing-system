    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">Unpaid Invoices</h1>
            {!isLoading && (
          ...
          <div className="flex items-center gap-2">
            {!isLoading && invoices.length > 0 && (
              <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-amber-500/10 text-amber-300 border border-amber-500/30">
                  ...
                  invoices.map((inv) => (
                    <tr key={inv.id} className="hover:bg-white/5 transition-colors">
                      <td className="px-6 py-4">
                        <code className="bg-white/5 text-slate-300 px-2 py-0.5 rounded text-xs font-mono">
                          {inv.invoice_number}
                        </code>
                      </td>
                      <td className="px-6 py-4 font-medium text-white">{inv.customer_name}</td>
