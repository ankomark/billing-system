 ...
 * Used by login.html when MikroTik requires CHAP-MD5 password hashing.
 *
 * Usage:
 *   var hashed = MD5(chap_id + password + chap_challenge);
 *   // Submit hashed value as the password field.
 *
 * Standard RSA Data Security MD5 Message-Digest Algorithm.
 */
...
function rstr2any(input, encoding) {
  ...
  var output = "";
  for (var i = remainders.length - 1; i >= 0; i--) output += encoding.charAt(remainders[i]);
  for (var i = 0; i < dividend.length - 1; i++) output = encoding[0] + output;
  return output;
}
...
function str2rstr_utf8(input) {
  ...
  while (++i < input.length) {
    ...
    if (x <= 0x007F) output += String.fromCharCode(x);
    else if (x <= 0x07FF) output += String.fromCharCode(0xC0 | ((x >>> 6) & 0x1F), 0x80 | (x & 0x3F));
